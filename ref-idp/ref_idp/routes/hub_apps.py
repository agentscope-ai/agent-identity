...
Mints a ``hub_<6hex>`` client_id for a resource server. Not persisted: this
reference IdP does not enforce audience registration at the token endpoint, so
a generated client_id is enough to exercise the end-to-end flow (the agent
requests a token for this client_id as the audience).
