...
sequenceDiagram
    ...
    participant CLI as AIP CLI / 控制台
    participant IdP as AIP 身份提供方
...
**为什么主体用 OAuth、智能体用密钥？** 主体是人（或人管理的组织）——有浏览器、能点"授权"、已经有 GitHub、Google 等账号。OAuth 是验证人类身份的合适工具。智能体是软件——没有浏览器、没有密码、自主运行。Ed25519 密钥认证是验证软件身份的合适工具。AIP 在每个环节使用最适合的认证方式。

```mermaid
graph TD
    P1["个人主体<br/>GitHub: @alice"]
    P1 --> A1["智能体: shark<br/>aip:alibaba:agent_7x8k2m"]
    P1 --> A2["智能体: experimental<br/>aip:alibaba:agent_2w6j8r"]

    P2["组织主体<br/>Acme Corp (已验证域名)"]
    P2 --> Admins["管理员: @alice, @bob, @carol"]
    P2 --> A3["智能体: acme-trader<br/>aip:alibaba:agent_9p3n5q"]
    P2 --> A4["智能体: acme-monitor<br/>aip:alibaba:agent_4k7m1w"]
```
...
```mermaid
graph LR
    Act1["行为"] --> Ag1["智能体<br/>aip:alibaba:agent_7x8k2m"] --> Pr1["主体: 个人<br/>GitHub @alice"] --> J1["管辖区域<br/>(US, CN 等, 可选)"]

    Act2["行为"] --> Ag2["智能体<br/>aip:alibaba:agent_9p3n5q"] --> Pr2["主体: 组织<br/>Acme Corp"] --> J2["管辖区域<br/>(US, 可选)"]
```
...
AIP 令牌是用 IdP 签名密钥签发的 JWT（RFC 7519）。注意：不是用智能体的密钥签。流程是：智能体用自己的私钥向 IdP 证明身份，IdP 再用 IdP 自己的密钥签发 JWT。服务方拿 IdP 的公钥来验证这个 JWT。
...
服务方维护一份可信 IdP 域名白名单。任何实现了 AIP 发现端点的 IdP 都是兼容的。
...
**AIP 信任计划：** 为了避免碎片化（每个服务方各自决定信任哪些 IdP），AIP 生态维护一份共享的信任列表——AIP 信任计划。参照 TLS CA（证书颁发机构）模式：

- 由一个开放治理机构维护可信 IdP 列表
- IdP 必须达到要求才能加入：技术合规（通过 AIP 测试套件）、运维安全（HSM 硬件保护签名密钥）、主体验证（已验证开发者/组织身份）、吊销能力、事件响应流程
...
**升级路径：** 如果智能体之后用同一密钥对在 IdP 注册，就可以把本地模式身份关联到 AIP 身份——无需重新注册即可升级。密钥不变，变的只是信任外壳。
...
AIP 在令牌中定义阈值，在协议中定义许可交互模式。服务方实施策略执行和主体通知。在基线（服务方本地）流程中，IdP 在许可签发时不参与——它在令牌签发时已提供了身份和声明。第 7.6.7 节定义了一个可选扩展：IdP 可以参与决策环节（但不参与策略评估）。
...
> 本扩展在真实企业部署下如何落地——一个主体、三个分属不同平台团队的服务方、推送式审批——参见配套文档 [审批场景](./2026-04-23-approval-scenarios.zh.md)。

第 7.6.1–7.6.6 节描述的基线模式中，服务方持有审批状态。这种模式适合跨组织联邦——每个服务方独立做策略评估。但对于运营自己 IdP 的组织来说，通常希望获得统一的审批体验：一个队列汇集所有服务方的待处理决策，一条审计链，一套送达偏好（门户、推送、邮件）。本节定义一个可选扩展：服务方将**决策环节**委托给主体的 IdP，但策略评估和执行仍由服务方本地完成。

**委托的边界。** 服务方继续决定_何时_需要审批、_什么_约束适用于所产生的许可——策略留在服务方。IdP 负责_谁_来决策以及_如何_与请求交互。IdP 在这种模式下是决策路由与签名服务，不是策略引擎。

##### 7.6.7.1 服务发现

支持委托审批的 IdP 在发现文档中声明 `approval_endpoint`：
...
信任该 IdP 的服务方应当（SHOULD）使用声明的 `approval_endpoint` 处理原本会进入本地流程的请求。服务方可以（MAY）按请求决定退回到本地流程（例如策略需要 IdP 通用界面无法呈现的上下文）。如果发现文档未声明 `approval_endpoint`，服务方必须（MUST）回退到基线流程（7.6.2）。
...
sequenceDiagram
    participant Agent as 智能体
    participant Hub as 服务方
    participant IdP
    participant Principal as 主体
    ...
    IdP->>Principal: 呈现请求（门户/推送/Webhook）
    Principal->>IdP: 批准或拒绝
    IdP->>IdP: 签发决策 JWT
    Agent->>Hub: 轮询 /aip/grants/{id}
    Hub->>IdP: 轮询 /aip/approvals/{id}
    IdP-->>Hub: {status, decision_jwt}
    Hub->>Hub: 用 JWKS 验证 JWT → 物化许可
    Hub-->>Agent: 许可
```

智能体看到的接口与基线流程完全一致（202 + 轮询 + 带 `X-AIP-Grant` 重试）。委托对智能体是透明的。

**第 1 步——服务方提交。** 服务方把请求细节 POST 到 IdP：
...

{
  ...
  "reason": "金额超过确认阈值（500 USD）"
}
```

IdP 查出智能体对应的主体，创建一条待处理审批，返回 `approval_id`。IdP 可以（MAY）验证服务方身份（mTLS、已注册的 hub_id、签名请求）；AIP 不强制具体机制。

**第 2 步——主体决策。** IdP 通过自己的渠道（门户、推送、短信）将请求呈现给主体。这部分用户体验超出 AIP 范围。

**第 3 步——IdP 签发决策。** IdP 使用签发智能体令牌的同一把密钥签发决策 JWT。任何已经信任该 IdP 的服务方都能验证它——无需新密钥、无需新信任根。
...
拒绝时，`decision` 为 `"denied"`，附带 `reason`，省略 `constraints`。

**第 4 步——服务方物化许可。** 服务方轮询时拿到 JWT，用 IdP 的 JWKS 验证签名，确认 `aud` 匹配自身 URL、`sub` 匹配发起请求的智能体，然后用 JWT 中的 `constraints` 签发本地许可。之后的流程与基线相同——智能体带 `X-AIP-Grant` 重试，服务方执行单次使用。

##### 7.6.7.3 为什么决策要签名，而不是只回状态码

返回签名断言而不是裸 HTTP 状态码，带来三个状态码本身做不到的属性：

- **不可否认性。** 服务方保留 IdP 批准了该操作的密码学证据——对审计、纠纷处理、合规报告都有用。
- **传输无关性。** 无论决策是通过轮询、Webhook 还是 sidecar 到达，JWT 都保持有效。
- **延迟验证。** 数月后复查时，只要保留了 JWKS 历史，第三方无需再联系 IdP 即可重新验证。

##### 7.6.7.4 委托审批的端点

支持委托审批的 IdP 应当（SHOULD）暴露：

| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/aip/approvals` | 服务方提交审批请求 |
| `GET` | `/aip/approvals/{approval_id}` | 服务方轮询获取签名决策 |

IdP 自己面向主体的端点（门户列表、UI 上的批准/拒绝操作）是 IdP 的内部实现，不属于本规范。`/aip/approvals` 前缀是约定——IdP 可以（MAY）使用不同路径，只要发现文档中的 `approval_endpoint` 指向它即可。

##### 7.6.7.5 为什么这不会破坏身份层与策略层的边界

委托审批一旦跑通，很容易想把更多策略塞进 IdP——时段判断、IP 白名单、资源标签策略。**不要这么做。** AIP 分层的价值在于：同一个智能体可以接入不同组织运营、使用不同策略引擎的服务方。一旦 IdP 变成策略引擎，每朵云上的每个服务方都要说同一种策略语言——联邦就崩了。

底线：IdP 路由_人类_决策并签名。机器能自动评估的（阈值、IP 规则、速率限制）都留在服务方。
...
| `/aip/activity/{agent_id}/summary` | GET | 任何 AIP 令牌（公开） | 聚合的公开统计 |
| `/aip/services` | POST | 服务方令牌 | 注册服务方 |
| `/aip/services/{service_id}` | GET | 任何 AIP 令牌 | 获取服务方信息 |
...
任何服务都可以验证 AIP 令牌。SDK 很轻量——验证一个 JWT，提取声明，就这么简单。
...
AIP 不与 MCP（Model Context Protocol）或 Agent Skills 竞争。它们工作在智能体技术栈的不同层次上，是互补关系。
...
block-beta
    ...
    AIP["AIP (Ed25519 + JWT) — '我是谁' — 跨平台身份，智能体原生认证"]
```

Skill 说「帮我订下周二去东京最便宜的机票」。MCP Tool 提供航空公司预订 API。AIP 回答「为什么航空公司应该让这个智能体刷我的卡——它是 travel-bot，由 Acme 公司运营，信任评分 0.85，受用户 Bob 委托，消费限额 5000 美元」。
...
AIP 和 MCP 可以协同工作。持有 AIP 身份的智能体仍然可以用 MCP OAuth 访问特定 MCP 服务器——AIP 提供 MCP 所缺少的持久身份层。
...
sequenceDiagram
    ...
    Skill->>Agent: "订东京的机票"
    Agent->>Hub: AIP JWT (自动)<br/>+ delegation: user_bob<br/>+ max_spend: 5000
...
**桥接 AIP 和 MCP OAuth** —— 对于只支持 MCP OAuth 的服务，智能体可以将 AIP JWT 出示给一个接受 AIP 令牌作为授权类型的 MCP 授权服务器（类似 OAuth 2.0 Token Exchange，RFC 8693）。授权服务器签发一个 MCP 范围的访问令牌。这在不要求服务原生实现 AIP 的前提下桥接了两个世界。

### 12.6 对比

| | Agent Skills | MCP Tools (OAuth 2.1) | AIP |
|---|---|---|---|
...
1. **命名** —— 「Agent Identity Protocol」（AIP）是工作名称。应该更具体还是更通用？会不会跟已有的「AIP」用法冲突？

2. **持久连接上的令牌刷新** —— SSE/WebSocket 连接的寿命可能超过令牌 TTL。服务方应该在初始 AIP 认证后用自己的会话令牌？还是 AIP 应该定义一个刷新机制？
...
8. **本地模式滥用** —— 本地模式没有问责链。服务方是否应该对本地模式的智能体实施不同的限流或沙盒策略？是否应该有一个标准的「升级到完整 AIP」提示？

9. **信任计划冷启动** —— 谁来召集 AIP 信任计划的初始治理机构？第一批成员怎么选？在生态还小的时候如何防止被单一厂商把控？
...
1. **在另一个 IdP 重新注册** —— 智能体用同一把公钥在新提供方注册，获得新的 agent_id（如 `aip:github.com:agent_xyz`）。智能体可以用共享密钥签署关联声明来证明连续性（见 6.6 节）。
...
这是 AIP 与平台拥有的身份模型（Ping Identity、Microsoft Entra）的关键差异：在 AIP 中，智能体能存活过它的提供方。私钥是身份的根，而不是 IdP 注册。
