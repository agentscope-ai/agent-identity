...
- **Resource server modes:**
  - `"disabled"`: bearer-only (default; back-compat).
  - `"optional"`: accept either scheme; verify DPoP proof when token + header are present.
  - `"required"`: refuse bearer; DPoP scheme + DPoP header mandatory.

Phase rollout: Phase A added `cnf.jkt` to every issued JWT (back-compat — old clients ignore unknown claims). Phase B-C tightens hubs to `dpop_mode="required"`; Bearer scheme deprecated.
...
On key revocation, the IdP bumps the agent's `token_version`. Resource servers learn of the bump either via deployment-supplied lookup callable (sync/async) or via a future bulk-sync endpoint. The SDK ships the enforcement hook (`Verifier(min_agent_token_version=lookup)`); the lookup mechanism is the deployment's choice — Phase C may add bulk-sync alongside the existing trust-list polling.
