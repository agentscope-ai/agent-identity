...
### 3.5 Hub Trust Tiers (v0.5)

Hubs are first-class principals — peers to IdPs, not registered agents (see [activity-discovery §2.0](./2026-05-04-activity-discovery.en.md)). But IdPs still need to express *which* hubs they're willing to mint tokens for and accept approval submissions from. Each IdP maintains its own per-IdP trust list with three states:

- **`verified`** — operator-reviewed (manifest audit, attester chain, KYB-equivalent). High-security deployments can require this tier.
- **`self_onboarded`** — auto-discovered on first sight of an audience: manifest fetched + JWS verified + namespace ownership checked, but not human-reviewed.
- **`blocked`** — operator-refused. Token issuance and approval submission refused (403).

Per-IdP, not central. The web doesn't have a global CA registry; each browser ships its own trust store, and that scales fine. AgentID adopts the same pattern.

Trust enforcement at activity ingest uses the same list — `aip-activity` polls the IdP's public trust-list endpoint and refuses events from hubs not in the cached set. This closes the "any domain with a manifest can pollute the activity stream" hole.

See [v0.5 design doc §3](./2026-05-20-hub-trust-and-dpop.en.md#3-hub-trust-tiers) for the full state machine, SSRF rails on auto-discovery, federation via `attested_by`, and the trust-list API contract.
...
sequenceDiagram
    ...
    CLI->>CLI: Save to ~/.agentid/config.json
```
...
```mermaid
graph TD
    Alice["👤 Human principal<br/>GitHub: @alice"] --> Shark["🤖 shark<br/>agentid:alibaba:agent_7x8k2m"]
    Alice --> Exp["🤖 experimental<br/>agentid:alibaba:agent_2w6j8r"]

    Acme["🏢 Org principal<br/>Acme Corp (verified domain)"] --> Trader["🤖 acme-trader<br/>agentid:alibaba:agent_9p3n5q"]
    Acme --> Monitor["🤖 acme-monitor<br/>agentid:alibaba:agent_4k7m1w"]
...
```mermaid
graph LR
    A1["Action"] --> Ag1["Agent<br/>agentid:alibaba:agent_7x8k2m"] --> P1["Principal: human<br/>@alice on GitHub"] --> J1["Jurisdiction<br/>(US, CN, etc.)"]
    A2["Action"] --> Ag2["Agent<br/>agentid:alibaba:agent_9p3n5q"] --> P2["Principal: org<br/>Acme Corp"] --> J2["Jurisdiction<br/>(US)"]
```
...
```mermaid
graph LR
    Agent["Agent<br/>(one keypair)"] --> QwenPaw["QwenPaw<br/>agentid:qwenpaw.ai:agent_abc<br/>JWT signed by QwenPaw"]
    Agent --> GitHub["GitHub<br/>agentid:github.com:agent_xyz<br/>JWT signed by GitHub"]
...
### 6.9 Sender-Constrained Tokens — DPoP (v0.5)

The bearer-token model in §6.2 has a structural weakness: any holder of a JWT can use it. JWTs travel widely (HTTP headers, logs, telemetry, IPC) and a leak == full identity compromise for the TTL window. RFC 9449 — Demonstrating Proof of Possession (DPoP) — fixes this by binding the token to a key pair: the JWT carries `cnf.jkt` (RFC 7638 thumbprint of the holder's public key) signed by the IdP; the holder presents a freshly-signed `DPoP` header on each request committing to method + URL + timestamp + token hash; the resource server checks that the proof's embedded JWK matches the token's `cnf.jkt`.

Stolen-JWT replay is structurally defeated. The thief lacks the private key, so they can't produce a valid `DPoP` header whose thumbprint matches the token's `cnf.jkt`.

AgentID's DPoP profile:

- **Algorithms:** `EdDSA` (Ed25519) MUST be supported; `ES256` (ECDSA P-256) MAY be. No others.
- **`cnf.jkt`:** RFC 7638 SHA-256 thumbprint of the canonical Ed25519 JWK. The IdP includes it on every issued JWT.
- **DPoP key:** the agent's existing IdP-registered key (no separate ephemeral DPoP key in v1).
- **Resource server modes:**
  - `"disabled"`: bearer-only. Used by deployments that have deliberately opted out (legacy traffic only).
  - `"optional"` (v0.6 default): accept either scheme; verify DPoP proof when token + header are present. Verifiers SHOULD log a one-time deprecation warning per `agent_id` when a Bearer request arrives whose token carries `cnf.jkt` — that combination signals the client could use DPoP but isn't.
  - `"required"`: refuse bearer; DPoP scheme + DPoP header mandatory.

Phase rollout:
- **Phase A (v0.5):** added `cnf.jkt` to every issued JWT (back-compat — old clients ignore unknown claims). SDK default `dpop_mode="disabled"`.
- **Phase B (v0.6):** SDK default flips to `dpop_mode="optional"`. Hubs constructed without an explicit mode get opportunistic DPoP coverage. Bearer scheme is **deprecated**: it keeps working, but verifiers emit a one-shot warning per agent that presents Bearer with a `cnf.jkt`-bearing token. Clients SHOULD opt into the `DPoP` scheme; demo-agent and demo-hub do by default.
- **Phase C (future):** target `dpop_mode="required"` for hubs handling sensitive flows (financial, irreversible). Bearer scheme MAY be removed entirely in a later version once telemetry shows ecosystem migration.

See [v0.5 design doc §6](./2026-05-20-hub-trust-and-dpop.en.md#6-dpop--sender-constrained-tokens) for the full RFC 9449 mapping, replay-cache obligations, and the mTLS-vs-DPoP tradeoff.

### 6.10 Compromise Recovery — `agent_token_version` (v0.5)

Key revocation alone doesn't kill in-flight JWTs — they remain valid until TTL expiry, leaving an unbounded compromise window. AgentID adds an `agent_token_version` claim mirroring the principal-side primitive from spec §6.4: every issued JWT carries the agent's current version; resource-server verifiers refuse JWTs whose version is below the agent's current.

On key revocation, the IdP bumps the agent's `token_version`. The operator MAY also bump it manually (a "panic-rotate" affordance) when key compromise is suspected but the scope is still unclear. The reference IdP exposes both paths: automatic bump on `DELETE /agentid/agents/{id}/keys/{kid}`, manual bump on `POST /agentid/agents/{id}/invalidate-tokens`.

Resource servers learn of the bump via:

- **Deployment-supplied lookup callable** (sync/async) — `Verifier(min_agent_token_version=lookup)`. Highest flexibility; the deployment chooses where the data lives.
- **Bulk-sync endpoint** (v0.6) — `GET /agentid/agents/token-versions`. Returns a snapshot of every agent whose version is non-zero (the default is implicit, keeping the wire payload bounded by revocation churn). Public-by-design — agent IDs are already public in JWT `sub` claims, and the only data exposed beyond that is a per-agent integer counter. Resource servers SHOULD poll on a tight cadence (the reference activity service polls every 60s) so the invalidation lag stays short. Fail-open semantics: if the snapshot is unreachable or stale, the lookup returns `None` and enforcement is skipped — every absent agent_id is implicitly at version 0 anyway, so the cache exists only to raise the floor for rotated agents.
...
**Approval policy DSL (v0.5)** — `requires_confirmation_above` was a single-knob hint. v0.5 generalises it to a small DSL composed by the IdP from three sources:

- Hub's published policy in its signed manifest (`approval_policy` field).
- Principal's per-(principal, hub) preferences set in the IdP portal.
- IdP-side tier overlay (v0.5 is pass-through; future revs may apply per-tier defaults).

The composed claim lives at `delegation` in the JWT:
...
Composition rule: **most-restrictive wins**. `always_require` unions across sources; numeric thresholds take the min; `never_require` (principal-only) is stripped of anything covered by `always_require` — hub-mandated approvals override principal opt-outs because regulatory floors aren't negotiable.

See [v0.5 design doc §4](./2026-05-20-hub-trust-and-dpop.en.md#4-hub-manifest-extensions) for the full DSL semantics and the IdP-side composition algorithm. Service-side evaluation lives in `agent-id-service-sdk.evaluate_approval_needed`.
...
#### 7.6.8 Approval-Mode Negotiation (v0.5)

Sections 7.6.1–7.6.6 describe Model 1 (hub-local approval state). Section 7.6.7 describes Model 3 (IdP-delegated decision). v0.5 adds a small negotiation layer so the choice between them isn't unilaterally the hub's:

Hubs publish which modes they implement in their signed manifest:
...
For v0.5 the existing discovery-doc fallback still applies (IdP advertises `approval_endpoint` → delegated; absent → local). The manifest fields are informational plumbing — they unlock per-principal preferences in v0.6+, where the principal can pick which mode they prefer per hub, and the IdP composes the preferred mode into a JWT claim alongside `delegation`.

See [v0.5 design doc §4.2](./2026-05-20-hub-trust-and-dpop.en.md#4-hub-manifest-extensions) for the field semantics and migration plan.
...
1. **Re-register with another IdP** — the agent registers the same public key with a new provider, getting a new agent_id (e.g., `agentid:github.com:agent_xyz`). The agent can prove continuity by signing a linkage statement with the shared key (Section 6.6).
