...
AIP 协议规范（§7.6 授权许可与审批工作流）的配套文档。**非规范性**——本文档不定义任何强制要求。它的作用是把两种审批模式落在一个真实部署里，让读者理解协议_为什么_长成这样。
...
**Rita** 是 **Acme 公司**的一位资深工程师。Acme 在 `idp.acme.com` 运营自己的 AIP 身份提供方，由 Acme 身份团队负责，接入了公司的 SSO。每位 Acme 员工在上面都已经有账号。

公司里三个内部平台团队各自运营一个 AIP 服务方：
...
没有任何审批流程触发。每个服务方用各自的策略评估令牌中的声明，自主决定。这是 AIP 的基线路径——第 1 层声明 + 服务方本地执行，没有人工参与。
...
DevOps 做一个审批 UI。数据团队做另一个。SRE 做另一个。Rita 有三个收件箱、三种登录流、三套界面。她经理配置 PTO 期间的代审批要在三个地方各配一次。等 Acme 规模扩大（更多团队接入 AIP，服务方数量上到几十个），方案在自身复杂度下崩溃。
...
IdP 委托审批不会破坏 AIP 联邦叙事，原因在于它严守的职责切分：
