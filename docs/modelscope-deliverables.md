...
| 2 | AgentID **Service SDK** doc (hub 接入) | [`agentid-service-sdk.md`](./agentid-service-sdk.md) | ✅ hub registration + verified endpoints |
...
pre.modelscope.cn (real agent `agent_id:modelscope:agent_…` + hub `hub_748233`):

- ✅ **Live endpoints** (discovery / JWKS / `agent_id/token` / `hub_apps`,
  resolved 2026-06-26; issuer `https://pre.modelscope.cn/openapi/v1`).
- ✅ **Hub `client_id`** — registered via the console (Identity Interconnection);
...
- ✅ SDK runtime + provider/verifier code + ref-idp hub enforcement (PRs:
