# AgentID Service SDK（IDA 侧）

`agent-id-service-sdk` 让 **Agent身份互联应用（Agent Identity Connected App,
IDA）** 验证 Agent 请求中携带的 AgentID JWT。IDA 是资源服务器，例如 API
gateway 或应用后端。SDK 会用 IdP 发布的公钥检查签名，并校验 issuer、audience、
过期时间，最后返回调用方身份。

这是 IDA 侧文档。Agent 侧通过
[`agent-id-client-sdk`](./agentid-client-sdk.zh.md) 获取 token。下面的示例使用
已上线的 ModelScope IdP，作为参考 IdP 实现。AgentID 不假设 ModelScope 是唯一
IdP。
...
# 可选 setup-time helper：只有用 Python 注册 IDA 应用时才需要：
...
`agent-id-service-sdk` 是运行时验签依赖。IDA serving runtime **不需要**
...
IDA 应用，或直接调用 `POST /hub_apps`，可以跳过 `agent-id-client-sdk`。

---

## 使用 ModelScope 时的前置条件：注册 IDA，拿到 `client_id`

使用已上线的 ModelScope IdP 时，IDA 需要先在 ModelScope 注册。仅做 token
认证时，IDA 不需要先自发布 `.well-known` manifest 或 JWKS。你需要先把 IDA
注册成一个 ModelScope IDA 应用，ModelScope 会返回 `client_id`。这个
`client_id` 就是所有 Agent 获取 token 时必须填写的 **`aud`**，也是 IDA
验签时必须强制匹配的 `audience`。

其他兼容协议的 IdP 可以采用不同的 setup-time 注册模型，也可以使用 IDA 的
service URL 作为 audience。`Verifier(audience=...)` 应与签发 token 的 IdP
规则保持一致。

### A. ModelScope 控制台

在 ModelScope 控制台进入 **Agent Identity → Identity Interconnection**，
选择 **Create Application**。填写 IDA 应用名称和 homepage / service endpoint
后提交，并保存返回的 `client_id`（例如 `hub_4abb08`）。这个 `client_id`
就是 agents 获取 token 时填写的 audience，也是 IDA 构造
...
适合脚本化注册 IDA。它需要 ModelScope AccessToken，效果等价于直接调用
`POST /hub_apps`。
...
ida_app = provider.create_hub_app(
    app_name="MyIDA",
    app_homepage="https://ida.example.com",
)
print(ida_app.client_id)  # 例如 hub_4abb08，这就是 verifier 的 audience
```

> ModelScope 可选的 `POST /hub_apps/endpoints/validate` 会探测
> `/.well-known/manifest`。如果你的 IDA 只做被动 token 验签，这个探测不会通过。
...
> 一是 verified IDA 信任标识，证明你拥有 Service Endpoint 域名；二是后续活动上报，
> 让 IDA 发布自己的签名公钥。它不参与 Agent token 的签发或验签。如果你的 IDA
> 只是被动验签（不发布 manifest），只有要做 verified IDA 标识或活动上报时，
> 才需要补上 manifest/JWKS。

> `POST /hub_apps` 在没有 token 时返回 ModelScope envelope
>（`InvalidAuthentication`）；注册时需要传
...
```text
GET https://www.modelscope.cn/openapi/v1/agent_id/.well-known/agentid-configuration
```

| 字段 | 值 |
| --- | --- |
| `issuer` | `https://www.modelscope.cn/openapi/v1`。验签配置中按 host 写入 `trusted_providers`，即 `www.modelscope.cn` |
| `jwks_uri` | `https://www.modelscope.cn/openapi/v1/agent_id/.well-known/agentid-jwks`，用于 `jwks_urls` |
| `token_endpoint` | `https://www.modelscope.cn/openapi/v1/agent_id/token` |
| signing alg | `EdDSA`。发现文档中它是 JSON 字符串，不是数组；因为我们直接 pin `jwks_urls` 并跳过 discovery，所以这不影响验签 |
...
一个全新的 IDA 如果要接入已上线的 ModelScope IdP，最小路径是：

1. 在 ModelScope 控制台注册 IDA（**Agent Identity → Identity Interconnection
...
4. 确认调用方 agents 已经拥有 ModelScope AgentID 身份。Agent 注册与配置见
   [`agentid-client-sdk.zh.md`](./agentid-client-sdk.zh.md)。
5. 给 agent operator 明确四项信息：IDA API base URL、IDA `client_id` audience、
   ModelScope IdP base URL、以及认证格式 `Authorization: Bearer <jwt>`。
6. issuer 和 JWKS 配置必须一致。公开 ModelScope IdP 统一使用
   `www.modelscope.cn` 和 `https://www.modelscope.cn/openapi/v1`。混用 issuer
   host、JWKS URL 或 audience 会导致 issuer、JWKS 或 audience 校验失败。

如果通过 Python helper 或 OpenAPI 注册，使用的 ModelScope AccessToken 是
setup-time 管理凭证。不要放进 agent runtime 配置，也不要要求 agents 把它发给你的 IDA。
...
verifier = Verifier(
    ...
    audience="hub_4abb08",                    # 注册 IDA 后拿到的 client_id
...
3. **Audience**：JWT `aud` 必须等于 `audience`，也就是注册 IDA 后得到的 `client_id`。
...
IDA 的授权逻辑应基于 `agent_id`，不要依赖 principal/scopes。
...
| --- | --- |
| `trusted_providers` | `["www.modelscope.cn"]` |
| `audience` | 注册 IDA 后拿到的 `client_id`，例如 `"hub_4abb08"` |
...
需要单独配置 activity endpoint 与 IDA 签名密钥。
...
- 已支持：ModelScope discovery/JWKS 路径、IDA `client_id` audience 与 token 验签。
