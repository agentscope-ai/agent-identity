# AgentID Service SDK（IDA 侧，ModelScope 版）
...
## 前置条件：注册 IDA，拿到 `client_id`

在 ModelScope 路径下，ModelScope 是 IDA 身份的中心登记方。仅做 token
...
一个全新的 IDA 要开始服务 ModelScope AgentID agents，最小路径是：
...
- 已验证过：预发 discovery + JWKS 可达；IDA `client_id` 签发与真实 token 验签已跑通。
