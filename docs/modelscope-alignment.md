...
（Agent 侧）与 `agent-id-service-sdk`（Hub/验签侧）需直接改为讲 ModelScope 协议——
**不做兼容层、不保留 shim**。
...
| 6 | `_audience_from_url` (L142-146) | audience = 资源 origin URL | audience = Hub 注册的 `client_id`（`hub_xxxxxx`），后端严格校验、拒绝任意 URL。必须显式 `default_audience="hub_xxxxxx"`，不再用 origin 推导 |
| 7 | DPoP (`_attach_auth_headers`) | 支持 DPoP | ModelScope 暂不签 `cnf.jkt`，**已确认不做 DPoP**。固定 `dpop=False` |

---

## 二、agent-id-service-sdk（Hub / 验签侧）
...
- **注册/HubApp 鉴权**：`Authorization: Bearer <ModelScope AccessToken>`（`MS_TOKEN`），走上游 OpenAPI 网关，不经 SDK token 流程。
...
DojoZero 同时消费两个 SDK：网关（Hub，service SDK）+ agent-runner/客户端（client SDK）。
...
|---|---|---|
| **SDK**（基础） | §一 / §二 | — |
| **网关 / Hub**（`agent_id_service_sdk`） | `gateway/_agentid.py`：把 SDK 新增的 JWKS 解析参数（`jwks_urls`）透传进 `Verifier(...)`。`audience` 本就是透传 env 值、Verifier 不校验 URL 格式，无需改校验逻辑 | `DOJOZERO_AGENTID_TRUSTED_PROVIDERS=test.modelscope.cn`（prod 用 `www.modelscope.cn`）；`DOJOZERO_AGENTID_AUDIENCE=<我方 hub client_id>`（**语义变更**：原为网关 origin URL）；JWKS 覆盖指向 `/openapi/v1/agent_id/.well-known/agentid-jwks` |
...
| **ModelScope 注册引导**（ops，非代码） | — | ① 用 ModelScope AccessToken 把网关注册为 **hub_app** → 取得两端共用的 `client_id`；② 逐个注册 **agent** → 生成内含 `idp_url` + `audience=client_id` 的 profile |
...
| **DPoP / token-versions / hub trust-list / manifest discovery** | ModelScope 路径不涉及；hub publisher / HubJWS 签名钥仍属我方，仅活动上报开启时才用 |
